...
export const sendTaskEmailToLawyerNodeMailer = async (emailParams: any) => {
  ...
  const emailHtml = `
    ...
    <head>
      ...
      <style>
        ...
        .content .task-details { margin-top: 20px; padding: 15px; background-color: #f4f4f9; border-radius: 10px; border: 1px solid #dddddd; }
        ...
        .task-status { font-weight: bold; color: #e53e3e; }
      </style>
    </head>
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <div class="task-details">
            <p><strong>Task ID:</strong> ${emailParams.taskId}</p>
            <p><strong>Task Type:</strong> ${emailParams.taskType}</p>
            <p><strong>Task Name:</strong> ${emailParams.taskName}</p>
          </div>
          <p>Please log into your Lawspicious dashboard for more details and to take further action.</p>
          <p class="task-status">This task is marked as <strong>${emailParams.taskStatus}</strong>.</p>
        </div>
        <div class="footer">
          <p>Best regards,</p>
  ...
  const mailOptions = {
    ...
    subject: `${emailParams.message.heading} - task ID: ${emailParams.taskId}`,
    html: emailHtml,
  };
